...
export default function App(){
  ...
  return(
    <>
      ...
      {tracks.map(t=>t.file&&(
        <audio key={t.id} ref={el=>audioRefs.current[t.id]=el} src={URL.createObjectURL(t.file)}
        ...
        <div style={{maxWidth:1100,margin:"0 auto",padding:"36px 28px 80px"}}>
          {!authed?(
            ...
            <>
              ...
              {/* Catalog table */}
              <div>
                <h3 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:2,color:D.text,marginBottom:12}}>Track Catalog</h3>
                <div style={{display:"flex",flexDirection:"column",gap:5}}>
                  {tracks.map(t=>(
                    <div key={t.id} style={{background:D.bg2,border:`1px solid ${D.border}`,borderRadius:7,padding:"11px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:14}}>
                      <div style={{display:"flex",gap:11,alignItems:"center",flex:1,minWidth:0}}>
                        <div style={{width:38,height:38,borderRadius:5,background:t.art,flexShrink:0}}/>
